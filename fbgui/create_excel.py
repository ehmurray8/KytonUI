...
class Table(ttk.Frame):
    ...
    def __init__(self, master):
        ...
        self.headers = ["File name", "Program Type"]
        self.tree = None
        self.item_ids = []
        ...
        try:
            ...
            paths = ["" for _ in range(len(names))]
            snums = ["" for _ in range(len(names))]

        self.prog_info = {"Name": names, "Type": types}
        self.file_paths = {name: path for name in self.prog_info["Name"] for path in paths}
        self.snums = {name: snum for name in self.prog_info["Name"] for snum in snums}
        conn.close()
        self._setup_widgets()
        self.setup_headers()
        for name, ptype in zip(self.prog_info["Name"], self.prog_info["Type"]):
            self.add_data([name, ptype])
    ...
    def _setup_widgets(self):
        ...
        create_xcel = ttk.Button(self, text="Create Excel Spreadsheet", command=self.create_spreadsheet)
